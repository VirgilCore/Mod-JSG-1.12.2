...
import tauri.dev.jsg.gui.container.machine.mcd.MCDContainerGuiUpdate;
...
public class MCDTile extends AbstractMachineTile {
    ...
    protected void playLoopSound(boolean stop) {
        JSGSoundHelper.playPositionedSound(world, pos, SoundPositionedEnum.MCD_LOOP, !stop);
    }

    @Override
    protected void playSound(boolean start) {
        if (!start)
            JSGSoundHelper.playSoundEvent(world, pos, SoundEventEnum.MCD_STOP);
        else
            JSGSoundHelper.playSoundEvent(world, pos, SoundEventEnum.MCD_START);
    }
